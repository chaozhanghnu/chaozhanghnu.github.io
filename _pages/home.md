...
<!-- 自动读取 _posts 文件夹中 category: news 的新闻，并按日期自动排序 -->
{% assign recent_news = site.categories.news %}
{% for post in recent_news %}
<p>
    <font size=4>
        {% if post.quick_news %}
            {{ post.news_prefix }} "<a href="{{ post.external_url }}">{{ post.title }}</a>" by {{ post.journal }}. -{{ post.date | date: "%m/%Y" }}
        {% else %}
            <a href="{{ post.url | relative_url }}">{{ post.title }}</a>
            <br>
            {{ post.excerpt | strip_html }} -{{ post.date | date: "%m/%Y" }}
        {% endif %}
    </font>
</p>
{% endfor %}
